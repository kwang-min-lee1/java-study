...
### JavaDoc
    - 자바 소스코드로부터 API 문서를 생성하는 표준도구
    - JDK 포함되어 있고, 소스코드에 포함된 특별한 형태의 주석을 HTML 문서로 변환
    - 목적
...
```java
        ...
        * @throws 예외에 대한 설명
        */
        public 리턴타입 메서드명 (파라미터타입 파리미터명) throws 예외명 {}
```        
    - JavaDoc 생성하기

```bash
javadoc -d 디렉토리명 소스파일명
```    
    - 'd' 문서가 생성될 때 디렉토리명을 설정
    - 한글을 위한 옵션
        - '-encoding': 인코딩을 UTF-8
        - '-charset' : 문자셋을 UTF-8
```bash
javadoc -encoding UTF-8 -charset UTF-8 -d 디렉토리명 소스파일명
```    
// 참고 연습  
// 자바 경로설정 후(환경설정에 bin 경로 주소 추가) 터미널에서 아래 명령을 실행  
// javadoc -encoding UTF-8 -charset UTF-8 -d docs src/javadoc/Polygon.javadoc -> <index-all.html> -> open in -> 브라우저  통해 확인  
...
## JAR 파일
    ...
    - 구성요소
        - Manifest 파일: JAR 파일의 루트에 META-INF 폴더 내에 필수정보와 메타데이터 포함.
